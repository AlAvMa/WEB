...
function cargarTabla(campo, orden){
    ...
    let url = "localhost:5000/contactos?id_usuario="+id+"&campo="+campo+"&oden="+orden;
...
function cargar_datos_tabla(res){
    ...
    for (let i=0; i<res.length; i++){
        ...
        col7.innerHTML = date.toLocalDateString() + " " + date.toLocaleTimeString();

        let col8 = document.createElement("td");
        col8.innerHTML = '<button onclick="editar_contacto('+res[i].id+');"style="cursor:pointer"><i class="fa fa-pen"></i></button>';

        let col9 = document.createElement("td");
        col9.innerHTML = '<button onclick="eliminar_contacto('+res[i].id+') style="cursor:pointer"><i class="fa fa-trash"></i></button>';
...
function eliminar(id_contacto){
    ...
    let url = "localhost:5000/contacto?id_usuario="+id_usuario+"&id_contacto="+id_contacto;
...
function buscar_contacto(){
    ...
    let url = "localhost:5000/contactoStr?id_usuario="+id+"&value="+value;
